...
```{r, include=FALSE}
train <- read_csv(here::here("posts", "machine_learning", "data", "train.csv")) |> 
  clean_names() |> 
  select(-c(id, x13))
test <- read_csv(here::here("posts", "machine_learning","data", "test.csv")) |> 
  clean_names() |> 
  mutate(ta1_x = ta1) |> 
  relocate(ta1_x, .after = si_o3u_m) |> 
  select(-c(id, ta1))

sample <- read_csv(here::here("posts", "machine_learning","data", "sample_submission.csv"))
